...
db = user.test 
sens = db.sens # database for sensors data
pwd = db.pwd   # database for password data
db.pwd.insert_one({"type": "pwd", "pwd": '66666666'})  # original password
...
while True:
    ...
    sens.insert_one(s1)
    time.sleep(2) # collecting data in every 2 seconds
    i += 1
    print(s1)
    if i > 1800: # collecting data in 1 hour (for testing)
